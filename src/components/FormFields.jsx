...
export const FormFileInput = ({
  ...
  const isVideoInput =
    Boolean(accept) &&
    String(accept).includes('video') &&
    !String(accept).includes('image');

  const looksLikeVideo = (url) => {
    if (!url || typeof url !== 'string') return false;
    if (url.startsWith('data:video/')) return true;
    if (url.startsWith('blob:') && isVideoInput) return true;
    if (/\.(mp4|webm|mov|ogg|m4v)(\?|#|$)/i.test(url)) return true;
    return false;
  };

  const isVideo = isVideoInput || looksLikeVideo(valueText);
    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="space-y-2">
        ...
        {isMultiple && valueText.length > 0 ? (
          ...
          <div className="relative min-h-[140px] overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-slate-100">
            {isVideo && !videoFailed ? (
              <video
                key={valueText}
                src={valueText}
                className="h-[140px] w-full bg-black object-contain"
                controls
                playsInline
                preload="metadata"
              ...
              <div className="flex h-[140px] w-full flex-col items-center justify-center bg-slate-900 px-4 text-center">
                <p className="text-sm font-semibold text-white">
                  Saved file is not a playable video
                </p>
                <p className="mt-1 text-xs text-white/70">
                  Upload a real MP4 / WEBM / MOV file
                </p>
              </div>
            ) : (
              <img
            ...
            <label
              htmlFor={fileInputId}
              className="absolute right-2 top-2 z-10 cursor-pointer rounded-lg bg-slate-900/80 px-3 py-1.5 text-xs font-bold text-white hover:bg-slate-900"
            >
