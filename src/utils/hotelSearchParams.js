...
export const mapUiFiltersToApi = (filters) => {
  ...
  const selectedFeatured = filters.selectedFeatured || [];
  const featuredTagSlugs = selectedFeatured.filter(
    (slug) => slug && slug !== "featured",
  );
  const tags = [...(filters.selectedTags || []), ...featuredTagSlugs].filter(
    Boolean,
  );
  ...
  return compactParams({
    ...
    isFeatured: selectedFeatured.includes("featured")
      ? true
      : filters.isFeatured
        ? true
        : undefined,
...
export const rankHotelSuggestions = (hotels = [], query = "", limit = 8) => {
  ...
  matches.sort((a, b) => {
    ...
    return String(a.hotel.label || "").localeCompare(
      String(b.hotel.label || ""),
    );
  });

  return [...matches.map((item) => item.hotel), ...recommendations].slice(
    0,
    limit,
  );
};
...
export const buildDestinationSuggestions = (items = [], query = "") => {
  ...
  items.forEach((hotel) => {
    ...
    const locationMatches =
      !q || (locationLabel && locationHaystack.includes(q));
    if (
      locationLabel &&
      locationMatches &&
